...
Your configuration for the \`assets\` option is \`${stringify(assets)}\`.`,
  ...
  EINVALIDASSETPACKAGEPROPERTY: ({ propertyName, propertyValue, verifyLink }) => ({
    message: `Invalid \`assets[*].package.${propertyName}\` option.`,
    details: `The [assets[*].package.${propertyName} option](${linkify(
      "README.md#assets"
    )}) must be a valid \`String\` and satisfy a specific format.
Your configuration for the \`assets[*].package.${propertyName}\` option is \`${stringify(propertyValue)}\`.
You may check the format at ${verifyLink}`,
  }),
